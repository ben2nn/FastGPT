...
const AppTreeSelect = ({
  ...
  placeholder?: string;
  fetchList: (parentId?: string | null, filterType?: 'workflow' | 'tool') => Promise<AppItem[]>;
  ...
  const { runAsync: loadChildren } = useRequest((parentId: string) => {
    ...
    return fetchList(parentId, filterType).finally(() =>
      setRequestingIds((prev) => prev.filter((id) => id !== parentId))
    );
  }, {});

  const { loading } = useRequest(() => fetchList(null, filterType), {
    manual: false,
    onSuccess: (data) => {
