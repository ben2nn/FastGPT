...
export const createImageDatasetCollection = async ({
  ...
  return await POST<{ collectionId: string }>(
    '/extend/dataset/collection/create/images',
    formData,
    {
      timeout: 600000,
      onUploadProgress: (e) => {
...
export const insertImagesToCollection = async ({
  ...
  return await POST<{ collectionId: string }>('/extend/dataset/data/insertImages', formData, {
    timeout: 600000,
    onUploadProgress: (e) => {
      if (!onUploadProgress) return;
      if (!e.progress) {
