...
import type { StatusStatisticsResponse } from '@/service/core/statistics/statistics';
...
async function handler(
  req: ApiRequestProps<{}, StatusQueryParams>,
  res: ApiResponseType<StatusStatisticsResponse>
) {
  ...
  try {
    ...
    /*  let tmbId: string;
         try {
             ...
             addLog.debug('[StatusAPI] 用户身份验证成功', { tmbId });
         } catch (error) {
             addLog.warn('[StatusAPI] 身份验证失败', {
  ...
  */
    ...
    // 4. 解析和验证查询参数
    addLog.debug('[StatusAPI] 原始查询参数', { query: req.query });

    let validatedQuery;
    try {
      validatedQuery = validateAndCleanQuery(req.query);
      addLog.debug('[StatusAPI] 验证后的查询参数', { validatedQuery });
    } catch (error) {
      if (error instanceof StatisticsError) {
        addLog.warn('[StatusAPI] 参数验证失败', {
          error: error.message,
    ...
    try {
      ...
      addLog.error('[StatusAPI] 统计服务初始化失败', error as Error);
    ...
    // 6. 调用统计服务获取数据
    let result: StatusStatisticsResponse;
    try {
      result = await statisticsService.getStatusStatistics(validatedQuery);
      addLog.info('[StatusAPI] 状态统计查询成功', {
        filters: {
        ...
        result: {
          ...
          distributionCount: result.distribution.length
        },
        duration: `${Date.now() - startTime}ms`
      });
    } catch (error) {
      if (error instanceof StatisticsError) {
        addLog.error('[StatusAPI] 统计查询失败', {
          code: error.code,
          message: error.message,
          details: error.details
        });
      ...
      addLog.error('[StatusAPI] 统计查询失败（未知错误）', error as Error);
      return res.status(500).json({
        code: StatisticsErrorCode.DATABASE_ERROR,
    ...
    // 统一错误处理（捕获未预期的错误）
    addLog.error('[StatusAPI] 未知错误', error as Error);
    return res.status(500).json({
      code: StatisticsErrorCode.DATABASE_ERROR,
