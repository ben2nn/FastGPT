 ...
 * 查询参数类型
 */
export type ModelStatisticsQueryParams = {
...
async function handler(
  req: ApiRequestProps<{}, ModelStatisticsQueryParams>,
  res: ApiResponseType<ModelStatisticsResponse>
) {
  ...
  try {
    ...
    try {
      ...
      addLog.debug('[ModelStatisticsAPI] 验证后的查询参数', { validatedQuery });
    } catch (error) {
      if (error instanceof StatisticsError) {
        addLog.warn('[ModelStatisticsAPI] 参数验证失败', {
          error: error.message,
    ...
    // 验证 pageSize 不超过最大值
    if (pageSize > 100) {
      addLog.warn('[ModelStatisticsAPI] pageSize 超过最大值', { pageSize });
      return res.status(400).json({
    ...
    // 验证 pageNum 为正整数
    if (pageNum < 1) {
      addLog.warn('[ModelStatisticsAPI] pageNum 无效', { pageNum });
      return res.status(400).json({
        code: StatisticsErrorCode.INVALID_PAGINATION,
    ...
    try {
      ...
      addLog.error('[ModelStatisticsAPI] 统计服务初始化失败', error as Error);
    ...
    // 7. 调用统计服务获取数据
    let result: ModelStatisticsResponse;
    try {
      result = await statisticsService.getModelStatistics(validatedQuery);
      addLog.info('[ModelStatisticsAPI] 按模型统计查询成功', {
        filters: {
      ...
      if (error instanceof StatisticsError) {
        addLog.error('[ModelStatisticsAPI] 统计查询失败', {
          code: error.code,
          message: error.message,
          details: error.details
        });
      ...
      addLog.error('[ModelStatisticsAPI] 统计查询失败（未知错误）', error as Error);
      return res.status(500).json({
        code: StatisticsErrorCode.DATABASE_ERROR,
    ...
    // 统一错误处理（捕获未预期的错误）
    addLog.error('[ModelStatisticsAPI] 未知错误', error as Error);
    return res.status(500).json({
      code: StatisticsErrorCode.DATABASE_ERROR,
