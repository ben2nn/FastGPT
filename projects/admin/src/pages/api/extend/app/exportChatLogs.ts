...
async function handler(req: ApiRequestProps<ExportChatLogsBody, {}>, res: NextApiResponse) {
  ...
  addLog.debug('[exportChatLogs] teamMemberWithContact', { count: teamMemberWithContact.length });
  ...
  cursor.on('data', (doc) => {
    addLog.debug('[exportChatLogs] processing doc', { chatId: doc.id });
    ...
    const res = `\n"${time}","${source}","${sourceName}","${tmbName}","${title}","${messageCount}","${userGoodFeedbackItemsStr}","${userBadFeedbackItemsStr}","${customFeedbackItemsStr}","${markItemsStr}","${chatDetailsStr}"`;
